...
fn render_and_extract_inner_inner<C>(init: C::Init, output: Option<PathBuf>)
where
    C: SimpleComponent,
    C::Root: IsA<gtk::Widget>,
    C::Init: Copy,
{
    ...
    app.connect_activate(move |app| {
        ...
        gtk::glib::idle_add_local_once(move || {
            let first = widget
                .first_child()
                .expect("render_and_extract_inner_inner: widget has no first child");

            let inner = first
                .first_child()
                .expect("render_and_extract_inner_inner: first child has no child");
...
fn main() {
    ...
    match cli.command {
        ...
        Command::SearchBarDefault => {
            render_and_extract_inner_inner::<cases::search_bar_default::SearchBarDefault>(
                (), output,
            )
        }
    }
